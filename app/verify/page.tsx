    ...
    try {
      ...
      const credRes = await fetch("/api/issue-credential", {
        ...
        body: JSON.stringify({
          ...
          countryCode: COUNTRIES[form.country],
          kycLevel: parseInt(form.kycLevel),
          dob: form.dob,
        }),
      });
      ...
      <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center px-6">
        <div className="max-w-lg w-full space-y-6 text-center">
          ...
          <div className="bg-gray-900 rounded-xl p-4 text-left space-y-1">
            {log.map((l, i) => (
              <p key={i} className="text-xs text-gray-400 font-mono">{l}</p>
            ))}
          </div>
        ...
        {(step === "proving" || step === "submitting") && (
          <div className="bg-gray-900 rounded-xl p-4 space-y-1">
            ...
            <p className="text-xs text-blue-400 font-mono animate-pulse">
              {step === "proving" ? "Generating proof..." : "Submitting to Soroban..."}
            </p>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                className="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-blue-500"
                value={form.kycLevel}
                onChange={(e) => setForm({ ...form, kycLevel: e.target.value })}
              >
            ...
            <p className="text-gray-600 text-xs text-center">
              Your personal data is never stored or sent to the blockchain.
              Only a ZK proof is submitted on-chain.
            </p>
          </div>
        )}
      </div>
